...
INT32 UIFlowWndMovie_OnKeyUp(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
   ...
   return NVTEVT_CONSUME;	
	    ...
	    case NVTEVT_KEY_PRESS:	
		    ...
		    if (Sensor_CheckExtSensor())
		    	{
		    	debug_msg("^RPipView_GetStyle() = %d",PipView_GetStyle());
			    switch(PipView_GetStyle())
			    {
                                ...
                                case PIP_STYLE_1T1F:
                                    PipView_SetStyle(PIP_STYLE_2T2F);// only A      
                                break;
                                
                                case PIP_STYLE_2T2F:
                                    PipView_SetStyle(PIP_STYLE_2T1B2S);// only A      
                                break;
                            #else
...
INT32 UIFlowWndMovie_OnCustom1(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	||(_MODEL_DSC_ == _MODEL_DUAL_SAIBOSHI_)||(_MODEL_DSC_ == _MODEL_CARDV_A9_))
    switch (uiKeyAct)
    { 
      case NVTEVT_KEY_PRESS:
	      if((KeyScan_GetPlugDev() != 1)&&(KeyScan_GetPlugDev() != 2))
